...
st.sidebar.header("4. Manual Design Parameters")
...
def run_analysis_dynamic(current_elements, current_nodes, optimized_slab_D, max_panel_x, max_panel_y):
    num_nodes = len(current_nodes)
    if num_nodes == 0: return current_elements, np.zeros(0), False
    
    F_global = np.zeros(num_nodes * 6)
    E_conc = 5000 * math.sqrt(fck) * 1e3
    G_conc = E_conc / 2.4 
    
    # Yield Line Load Distribution
    q_u = 1.5 * (live_load + floor_finish + (optimized_slab_D / 1000.0) * 25.0)
    ...
    seismic_mass_per_floor = ((q_u / 1.5) * (Lx * Ly)) + ((0.25 if live_load <= 3.0 else 0.50) * (live_load * Lx * Ly))
    ...
    for el in current_elements:
        ...
        if el['type'] == 'Beam':
            ...
            if dx > dy: slab_load = w_long if max_panel_x >= max_panel_y else w_short
            else: slab_load = w_long if max_panel_y > max_panel_x else w_short
            
            is_secondary = not (ni.get('is_primary', True) and nj.get('is_primary', True))
            h_story = (z_elevations.get(ni['floor'], 3.0) - z_elevations.get(ni['floor']-1, 0.0)) if ni['floor']>0 else 3.0
            wall_udl = (wall_thickness / 1000.0) * 20.0 * max(0.1, (h_story - h)) if not is_secondary else 0.0
            self_wt = b * h * 25.0 * 1.5
            
            el['load_kN_m'] = slab_load + wall_udl + self_wt
            seismic_weight_total += (wall_udl/1.5 + self_wt/1.5) * L
    ...
    try:
        ...
        unstable = True
        # Stabilizer to bypass full crash
        K_free = K_global[np.ix_(free_dofs, free_dofs)] + (np.eye(len(free_dofs)) * 1000.0)
        U_free = np.linalg.solve(K_free, F_global[free_dofs])
    ...
    for el in current_elements:
        ...
        dim_min, dim_max = min(b, h), max(b, h)
        J_sec = (dim_min**3 * dim_max) * (1/3 - 0.21 * (dim_min/dim_max) * (1 - (dim_min**4) / (12 * dim_max**4)))
        k_local = get_local_stiffness(E_conc, G_conc, b*h, (h*b**3)/12.0, (b*h**3)/12.0, J_sec, el['length'])
    ...
    return current_elements, U_global, unstable

def perform_design(elements_to_design, U_global, current_nodes, z_elevations, unstable):
    design_status = True
    ...
    for z in range(1, num_stories + 1):
        ...
        drift_x, drift_y = abs(max_x_z - max_x_prev), abs(max_y_z - max_y_prev)
    ...
    for el in elements_to_design:
        ...
        el['pass'] = True
        el['design_details'] = {}
        el['failure_mode'] = "Mechanism " if unstable else ""
        ...
        if el['type'] == 'Beam':
            ...
            max_deflection = delta_ss + abs((L_mm / 8) * (theta_1 - theta_2))
...
# --- EXECUTION BLOCK ---
if st.button("Run Full Analysis & Deep AI Optimization", type="primary", use_container_width=True):
    with st.spinner("Processing Structural Engine..."):
        ...
        max_panel_x = max(x_spans) if x_spans else 1.0
        max_panel_y = max(y_spans) if y_spans else 1.0
        ...
        manual_elements, U_man, m_unstable = run_analysis_dynamic(manual_elements, nodes, slab_thickness, max_panel_x, max_panel_y)
        manual_elements, man_passed = perform_design(manual_elements, U_man, nodes, z_elevations, m_unstable)
        ...
        while iteration <= 15:
            ai_elements, U_ai, ai_unstable = run_analysis_dynamic(ai_elements, nodes, opt_slab_thickness, max_panel_x, max_panel_y)
            ai_elements, ai_passed = perform_design(ai_elements, U_ai, nodes, z_elevations, ai_unstable)
            if ai_passed or ai_unstable: 
                break 
        ...
        # --- UI TABS ---
        if m_unstable:
            st.error("🚨 MATRIX SINGULARITY DETECTED: A column in your layout is completely unbraced. Example: Column C8 at Grid (C, 6) has no connecting beams in the X direction. Results are artificially stabilized and INVALID. Please fix geometry.")
        else:
        ...
        with tab_man:
            ...
            col_b.dataframe(pd.DataFrame([el['design_details'] for el in manual_elements if el['type'] == 'Beam']), use_container_width=True)
            col_c.dataframe(pd.DataFrame([el['design_details'] for el in manual_elements if el['type'] == 'Column']), use_container_width=True)

        with tab_ai:
            if ai_unstable:
                st.error("AI Optimization Aborted due to severe structural instability (Mechanism). Fix the plan grids.")
            else:
                ...
                col_b2.dataframe(pd.DataFrame([el['design_details'] for el in ai_elements if el['type'] == 'Beam']), use_container_width=True)
                col_c2.dataframe(pd.DataFrame([el['design_details'] for el in ai_elements if el['type'] == 'Column']), use_container_width=True)
                ...
                for n in deformed_nodes:
                    n["x"] += U_ai[n["id"] * 6] * scale
                    n["y"] += U_ai[n["id"] * 6 + 1] * scale
                    n["z"] += U_ai[n["id"] * 6 + 2] * scale
